...
export function ProofTab({ report, accent }: ProofTabProps) {
  ...
  // Best-model lift vs the rule baseline (the credible TSTR headline number
  // once transformers are trained on the leakage-safe pack split).
  const bestBar = bars.find((b) => b.best) ?? bars[bars.length - 1];
  const headlineLift = tstr?.transformer_best_model
    ? (bestBar.value - (tstr?.rule_strict_rare_recall ?? baseline))
    : (tstr?.recall_lift ?? 0);
            ...
            return (
              <g key={b.label}>
                <rect
                  ...
                  rx={8}
                  fill={b.color}
                  opacity={b.best ? 1 : 0.85}
                  stroke={b.best ? '#1d1d1f' : 'none'}
                  strokeWidth={b.best ? 2 : 0}
                />
                <text x={x + barW / 2} y={y - 8} textAnchor="middle" fontSize={13} fill="#1d1d1f" fontWeight="600">
                  {(b.value * 100).toFixed(0)}%
                </text>
                {b.best && (
                  <text x={x + barW / 2} y={y - 22} textAnchor="middle" fontSize={10} fill="#1d1d1f" fontWeight="700">
                    BEST
                  </text>
                )}
                <text x={x + barW / 2} y={chartH - 18} textAnchor="middle" fontSize={12} fill="#6e6e73">
                  {b.label}
                </text>
                {b.sublabel && (
